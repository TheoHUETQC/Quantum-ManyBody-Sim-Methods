...
for lambda_val in lambda_list:
    ...
    for nqubits in nqubits_list:
        ...
        for r in runs:
            re_series = get_data(r+1, nqubits, "Renyi_entropy", lambda_val, fixed_max_size)
            oe_series = get_data(r+1, nqubits, "Operator_Entanglement", lambda_val, fixed_maxdim)
    ...
    axes[0].errorbar(nqubits_list, re_means, yerr=re_errs, marker='o', label=f"λ={lambda_val:.2f}", color=color)
    axes[1].errorbar(nqubits_list, oe_means, yerr=oe_errs, marker='s', color=color)
