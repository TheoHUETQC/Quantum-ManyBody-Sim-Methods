...
def csv_to_df(run, nqubits, entropy) :
  ...
  path= f"/content/drive/MyDrive/resultat_simu_osaka/random_circuit_noisy/run_{run}/"+file_name
...
for nqubits in nqubits_list :
  ...
  for gamma in gamma_list :
    ...
    for run in range(1, runs+1) :
      ...
      renyi_entropy.append(csv_to_df(run, nqubits, "Renyi_entropy")["gamma=$gamma"])
      operator_entanglement.append(csv_to_df(run, nqubits, "Operator_Entanglement")["gamma=$gamma"])
